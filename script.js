		...
		const disassembleArm = disassembler.arm = (overlay, style, isArmv5) => {
			...
			const shifterOperand = (inst) => {
				...
				if ((inst & 0x0e000070) === 0x00000060) {
					const shiftImm = inst >>> 7 & 0x1f;
					const Rm = inst & 0xf;
					if (shiftImm !== 0) { // shiftImm === 0 is RRX
			...
			for (let i = 0; i < u32.length; ++i) {
				...
				if ((inst & 0x0f100010) === 0x0e000010) {
					...
					if (cond === conds[0b1111]) {
						...
						if (isArmv5) {
							if (ASM)
								lines.push(`mcr2 p${cpNum}, ${opcode1}, ${r[Rd]}, c${CRn}, c${CRm}` +
									(opcode2 ? `, ${opcode2}` : '') + u);
							continue;
						}
					} else {
						if (ASM) lines.push(`mcr${cond} p${cpNum}, ${opcode1}, ${r[Rd]}, c${CRn}, c${CRm}` +
							(opcode2 ? `, ${opcode2}` : '') + u);
						continue;
					}
				}
				...
				if ((inst & 0x0ff00090) === 0x01000080 && cond !== conds[0b1111]) {
					...
					const Rm = inst & 0xf;
					const u = unpredictable(Rd === 15 || Rm === 15 || Rn === 15 || Rs === 15);
					if (ASM) lines.push(`smla${x ? 't' : 'b'}${y ? 't' : 'b'}${cond} ${r[Rd]}, ${r[Rm]}, ` +
						`${r[Rs]}, ${r[Rn]}` + u);
					continue;
				}
				...
				if ((inst & 0x0fe000f0) === 0x00e00090 && cond !== conds[0b1111]) {
					...
					const Rs = inst >>> 8 & 0xf;
					const Rm = inst & 0xf;
					const u = unpredictable(RdHi === 15 || RdLo === 15 || Rm === 15 || Rs === 15 || RdHi === RdLo
						|| RdHi === Rm || RdLo === Rm);
				...
				if ((inst & 0x0ff00090) === 0x01400080 && isArmv5 && cond !== conds[0b1111]) {
					...
					if (ASM) lines.push(`smlal${x ? 't' : 'b'}${y ? 't' : 'b'}${cond} ${r[RdLo]}, ${r[RdHi]}, ` +
						`${r[Rm]}, ${r[Rs]}` + u);
					continue;
				}
				...
				if ((inst & 0x0fe000f0) === 0x00c00090 && cond !== conds[0b1111]) {
					...
					const Rs = inst >>> 8 & 0xf;
					const Rm = inst & 0xf;
					const u = unpredictable(RdLo === 15 || RdHi === 15 || Rm === 15 || Rs === 15 || RdHi === RdLo
						|| RdHi === Rm || RdLo === Rm);
				...
				if ((inst & 0x0e500000) === 0x08000000 && cond !== conds[0b1111]) { // (1) (A4.1.97)
					...
					const u = unpredictable(Rn === 15 || registers === 0 ||
						(W && (registers & (1 << Rn)) && (registers & ((1 << Rn) - 1))));
					if (ASM) {
						const list = [];
						for (let bit = 1, i = 0; i < 16; bit <<= 1, ++i) {
							if (registers & bit) list.push(r[i]);
